...
private:
    ...
    Twmv9dll *wmv9dll;
public:
    Tff_wmv9(void): wrinputnum(0), writing(false), nOutFrames(0), cbVideoOut(0), decRtLength(0), decmult(0)
	{
		ok = false;
		pProfile = NULL;
		m_pCodecInfo = NULL;
...
#define   JITTER_OFFSET            50000
    ...
    int writeAVI(unsigned int framenum, uint64_t csp, const unsigned char * const src[4], const stride_t srcStride[4], void *dst) {
        ...
        return (dwVideoStatus & DMO_OUTPUT_DATA_BUFFERF_SYNCPOINT) ? -(static_cast<int>(ulLength)) : ulLength;
    }
    void endAVI(void) {
