...
struct Trect {
	...
	operator RECT(void)	{
        RECT ret = {static_cast<LONG>(x), static_cast<LONG>(y), static_cast<LONG>(dx - 1), static_cast<LONG>(dy - 1)};
        return ret;
    }
	operator CRect(void) {
    ...
    static void calcNewSizeAspect(const Trect &in, int a1, int a2, Trect &out) {
        if ((in.dx << 16) / in.dy > (static_cast<unsigned int>(a1) << 16) / static_cast<unsigned int>(a2)) {
            out.dy = a2 * in.dx / a1;
            out.dx = in.dx;
        } else {
            out.dx = a1 * in.dy / a2;
