...
TffdshowPageEnc::TffdshowPageEnc(LPUNKNOWN pUnk, HRESULT *phr, const wchar_t *ItitleW, const char_t *name, int dialogId, int resstr)
	: TffdshowPageBase(pUnk, phr, ItitleW, name, dialogId, resstr),
	  oldCoSettingsPtr(nullptr), 
	  oldmode(0), 
      valIdff(0), 
      valMin(0), 
      valMax(0), 
...
HRESULT TffdshowPageEnc::OnConnect(IUnknown *pUnk)
{
	...
	HRESULT hr = deciE->getCoSettingsPtr(const_cast<const TcoSettings**>(&oldCoSettingsPtr));
	if (SUCCEEDED(hr)) {
		localCfg = oldCoSettingsPtr;
		deciE->setCoSettingsPtr(localCfg);
		hr = TffdshowPageBase::OnConnect(pUnk);
		if (SUCCEEDED(hr)) {
			...
			codecId = static_cast<AVCodecID>(cfgGet(IDFF_enc_codecId));
		}
	}
...
HRESULT TffdshowPageEnc::OnDisconnect(void)
{
    ...
    deciE->setCoSettingsPtr(static_cast<TcoSettings*>(oldCoSettingsPtr));
...
void TffdshowPageEnc::quick2dlg(bool redraw)
{
    ...
    if (ii == CB_ERR && cbxGetItemCount(IDC_CBX_MODES) > 0) {
        ii = 0;
        mode = static_cast<int>(cbxGetItemData(IDC_CBX_MODES, 0));
...
void TffdshowPageEnc::codec2dlg(void)
{
    ...
    for (int i = 0; i < cnt; i++)
        if (reinterpret_cast<Tencoder*>(cbxGetItemData(IDC_CBX_ENCODER, i))->id == codecId) {
...
void TffdshowPageEnc::fourcc2dlg(void)
{
    ...
    for (int i = 0; i < cnt; i++)
        if (static_cast<FOURCC>(cbxGetItemData(IDC_CBX_FOURCC, i)) == fourcc) {
            cbxSetCurSel(IDC_CBX_FOURCC, i);
            return;
        }
    cbxSetCurSel(IDC_CBX_FOURCC, 0);
    cfgSet(IDFF_enc_fourcc, static_cast<int>(cbxGetItemData(IDC_CBX_FOURCC, 0)));
}
            ...
            switch (LOWORD(wParam)) {
                case IDC_CBX_ENCODER:
                    if (HIWORD(wParam) == CBN_SELCHANGE) {
                        cfgSet(IDFF_enc_codecId, reinterpret_cast<Tencoder*>(cbxGetCurItemData(IDC_CBX_ENCODER))->id);
                        codec2dlg();
                ...
                case IDC_CBX_FOURCC:
                    if (HIWORD(wParam) == CBN_SELCHANGE) {
                        cfgSet(IDFF_enc_fourcc, static_cast<int>(cbxGetCurItemData(IDC_CBX_FOURCC)));
                        fourcc2dlg();
                        return TRUE;
                    }
                    break;
                case IDC_CBX_MODES:
                    if (HIWORD(wParam) == CBN_SELCHANGE) {
                        cfgSet(IDFF_enc_mode, static_cast<int>(cbxGetCurItemData(IDC_CBX_MODES)));
                        quick2dlg(true);
                        return TRUE;
                    }
                    break;
        ...
        case WM_DRAWITEM:
            if (wParam == IDC_CBX_ENCODER) {
                ...
                if (int(dis->itemData) != CB_ERR) {
                    RECT r = dis->rcItem;
                    r.left += 2;
                    Tencoder *enc = reinterpret_cast<Tencoder*>(dis->itemData);
                    DrawText(dis->hDC, enc->name, static_cast<int>(strlen(enc->name)), &r, DT_LEFT | DT_SINGLELINE | DT_VCENTER);
                    setDlgResult(TRUE);
                    r.left += 180;
                    FillRect(dis->hDC, &r, br);
                    MoveToEx(dis->hDC, r.left, r.top, nullptr);
                    ...
                    DrawText(dis->hDC, text, static_cast<int>(strlen(text)), &r, DT_LEFT | DT_SINGLELINE | DT_VCENTER);
                }
...
void TffdshowPageEnc::showPresets(void)
{
    ...
    int cmd = TrackPopupMenu(hm, TPM_LEFTALIGN | TPM_TOPALIGN | TPM_RETURNCMD, p.x, p.y, 0, m_hwnd, nullptr);
...
void CALLBACK configureEnc(HWND hwnd, HINSTANCE hinst, LPTSTR lpCmdLine, int nCmdShow)
{
    ...
    if (CoCreateInstance(CLSID_FFDSHOWENC, nullptr, CLSCTX_INPROC_SERVER, IID_IffdshowEnc, reinterpret_cast<void**>(&deciE)) == S_OK) {
