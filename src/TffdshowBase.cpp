    ...
    graph(Igraph)
{
    ...
    static const TintOptionT<TffdshowBase> iopts[] = {
        IDFF_alwaysOnTop  , &TffdshowBase::cfgDlgAlwaysOnTop , 0, 0, _l(""), false, nullptr, 0,
        IDFF_notreg       , &TffdshowBase::notreg            , 0, 0, _l(""), false, nullptr, 0,
        IDFF_defaultMerit , &TffdshowBase::defaultMerit      , -1, -1, _l(""), false, nullptr, 0,
        IDFF_applying     , &TffdshowBase::applying          , 0, 0, _l(""), false, nullptr, 0,
        0
    };
    addOptions(iopts);
...
void TffdshowBase::sendOnFrameMsg(void) const
{
    if (onFrameMsg) {
...
void TffdshowBase::onLangChange(int id, const char_t *newval) const
{
    if (trans) {
...
STDMETHODIMP TffdshowBase::dbgError(const char_t *fmt, ...)
{
    ...
    va_list args;
    va_start(args, fmt);
    _vsnprintf(msg, 260, fmt, args);
    va_end(args);
    dbgWrite(msg);
    if (globalSettings->errorbox) {
        MessageBox(nullptr, msg, _l("ffdshow error"), MB_ICONERROR | MB_OK);
    }
    return S_OK;
}
...
STDMETHODIMP TffdshowBase::getLibavcodec(Tlibavcodec* *libavcodecPtr)
{
    ...
    if (libavcodec->ok) {
        ...
        *libavcodecPtr = libavcodec;
        ...
        MessageBox(nullptr, _l("Failed to load ffmpeg.dll!"), _l("ffdshow error"), MB_ICONERROR | MB_OK);
        *libavcodecPtr = nullptr;
        return E_FAIL;
    }
}
...
void TffdshowBase::onTrayIconChange(int id, int newval)
{
    if (globalSettings->trayIcon && inExplorer() != S_OK)
        if (!trayHwnd) {
            unsigned threadID;
            CAMEvent ev;
            hTrayThread = reinterpret_cast<HANDLE>(_beginthreadex(nullptr, 128 * 1024, reinterpret_cast<unsigned(__stdcall *)(void*)>(trayIconStart), &std::make_tuple((IffdshowBase*)this, &ev, &trayHwnd), 0, &threadID));
            ev.Wait();
        }
}
...
HRESULT AddToRot(IUnknown *pUnkGraph, DWORD *pdwRegister)
{
    ...
    StringCchPrintfW(
        ...
        reinterpret_cast<DWORD_PTR>(pUnkGraph),
        GetCurrentProcessId()
    );
...
//tell and seek
double TffdshowBase::tell(void) const
{
    if (!graph) {
        return E_UNEXPECTED;
    }
...
STDMETHODIMP TffdshowBase::tell(int *seconds)
{
    ...
    *seconds = -1;
    return ((*seconds = static_cast<int>(tell() + 0.5)) != -1) ? S_OK : S_FALSE;
}
...
int TffdshowBase::getDuration(void) const
{
    if (!graph) {
        return E_UNEXPECTED;
    }
...
STDMETHODIMP TffdshowBase::frameStep(int diff)
{
    ...
    if (comptrQ<IMediaSeeking> ms = graph) {
        ...
        if (SUCCEEDED(hr)) {
            curpos += diff;
            hr = ms->SetPositions(&curpos, AM_SEEKING_AbsolutePositioning, nullptr, AM_SEEKING_NoPositioning);
        }
        if (oldtf != TIME_FORMAT_FRAME) {
...
void TffdshowBase::setPropsTime(IMediaSample *sample, REFERENCE_TIME t1, REFERENCE_TIME t2, AM_SAMPLE2_PROPERTIES* const pProps, BOOL *m_bSampleSkipped)
{
    if (comptrQ<IMediaSample2> pOutSample2 = sample) {
        ...
        EXECUTE_ASSERT(SUCCEEDED(pOutSample2->GetProperties(FIELD_OFFSET(AM_SAMPLE2_PROPERTIES, tStart), reinterpret_cast<PBYTE>(&OutProps))));
        ...
        pOutSample2->SetProperties(FIELD_OFFSET(AM_SAMPLE2_PROPERTIES, dwStreamId), reinterpret_cast<PBYTE>(&OutProps));
...
HRESULT TffdshowBase::getDeliveryBuffer(IMediaSample **pSample, BYTE **pData) const
{
    HRESULT hr;
